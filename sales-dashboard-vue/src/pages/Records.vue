  ...
  <div class="max-w-[1100px] mx-auto space-y-4">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-[11px] text-trust-300 mt-0.5 font-sans">{{ isSales ? '查看个人每日填报的业绩汇总' : '查看每日每人填报的业绩汇总' }}</p>
      </div>
      <div class="flex items-center gap-4">
        ...
        <div class="text-right">
          <p class="text-[10px] text-trust-300 font-sans">筛选结果 DGMV 合计</p>
          <p class="text-[18px] font-extrabold text-success-light font-mono tabular-nums">{{ fmt(summaryDgmv) }}</p>
        </div>
      </div>
    </div>
    <div class="bg-surface-raised rounded-2xl border border-white/[0.04] px-5 py-3.5 flex items-center gap-4">
      ...
      <template v-if="canViewAll">
        <label for="fk" class="sr-only">搜索姓名</label>
        <input id="fk" type="text" v-model="keyword" @input="debounceSearch" placeholder="搜索姓名..." class="bg-transparent text-[12px] text-gray-300 placeholder-gray-500 focus:outline-none font-sans w-28" />
        <div class="w-px h-5 bg-white/[0.06]" />
      </template>
      <label for="fs" class="sr-only">开始日期</label>
      <input id="fs" type="date" v-model="startDate" @change="fetchRecords" class="bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[11px] text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand/30 cursor-pointer font-mono [color-scheme:dark]" />
      <label for="fe" class="sr-only">结束日期</label>
      <input id="fe" type="date" v-model="endDate" @change="fetchRecords" class="bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[11px] text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand/30 cursor-pointer font-mono [color-scheme:dark]" />
    ...
    <div class="bg-surface-raised rounded-2xl border border-white/[0.04] overflow-hidden">
      <table class="w-full table-fixed">
        ...
        <tbody>
          <tr v-for="row in groupedRows" :key="row.key" class="border-b border-white/[0.02] hover:bg-white/[0.03] transition-colors duration-150">
            <td class="px-3 py-2.5 text-[11px] text-gray-300 font-mono tabular-nums whitespace-nowrap">{{ row.date }}</td>
            <td class="px-3 py-2.5">
              <div class="flex items-center gap-2">
                <div class="w-5 h-5 rounded-md bg-gradient-to-br from-brand to-brand-light flex items-center justify-center text-white text-[8px] font-semibold font-mono shrink-0">{{ (row.name || '?')[0] }}</div>
                <span class="text-[11px] font-medium text-gray-300 font-sans truncate">{{ row.name }}</span>
              </div>
            </td>
            <td class="px-3 py-2.5 text-[11px] text-gray-300 font-mono tabular-nums text-right">{{ fmt(row.gmv) }}</td>
            <td class="px-3 py-2.5 text-[11px] text-danger-light/70 font-mono tabular-nums text-right">-{{ fmt(row.refund) }}</td>
            <td class="px-3 py-2.5 text-[12px] text-success-light font-mono tabular-nums font-bold text-right">{{ fmt(row.dgmv) }}</td>
            <td class="px-3 py-2.5 text-center">
              <router-link :to="{ name: 'recordDetail', params: { userId: row.userId }, query: { date: row.date } }" class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium text-brand-light hover:text-white hover:bg-brand/[0.1] transition-colors cursor-pointer font-sans">
      ...
      <div v-else-if="groupedRows.length === 0" class="py-14 text-center text-trust-300 text-[12px] font-sans">暂无匹配记录</div>
    ...
    <div v-if="showWebhookModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showWebhookModal = false">
      <div class="bg-surface-raised rounded-2xl border border-white/[0.06] w-[480px] p-6 space-y-4">
        ...
        <p class="text-[11px] text-trust-300 font-sans leading-relaxed">配置飞书机器人 Webhook 地址后，系统将在每天上午 9:00 和 10:00 自动推送前一天未填报人员提醒到飞书群。</p>
        <div>
          <label for="wh-url" class="block text-[11px] text-trust-300 font-sans mb-1.5">Webhook 地址</label>
          <input id="wh-url" type="text" v-model="webhookUrl" placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..." class="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-2 text-[12px] text-gray-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand/30 font-mono" />
        </div>
...
const columns = [
  ...
  { key: 'gmv', label: 'GMV', sortable: true, align: 'right' },
  { key: 'refund', label: '退款', sortable: true, align: 'right' },
  { key: 'dgmv', label: 'DGMV', sortable: true, align: 'right' },
...
const allGroupedRows = computed(() => {
  ...
  for (const r of records.value) {
    ...
    if (!map.has(key)) {
      map.set(key, { key, date: r.recordDate, userId: r.userId, name: r.userName || auth.user?.name || String(r.userId), gmv: 0, refund: 0, dgmv: 0 })
    }
    const g = map.get(key)
    g.gmv += Number(r.gmv) || 0
    g.refund += Number(r.refund) || 0
    g.dgmv += Number(r.dgmv) || 0
  }
  let rows = [...map.values()]
  // 前端排序
  if (sortField.value) {
    ...
    rows.sort((a, b) => {
      const va = sortField.value === 'date' ? a.date : a[sortField.value]
      const vb = sortField.value === 'date' ? b.date : b[sortField.value]
...
async function fetchRecords() {
  ...
  try {
    ...
    const sp = { startDate: startDate.value || undefined, endDate: endDate.value || undefined, keyword: keyword.value || undefined }
    const [rr, sr] = await Promise.all([api.get('/records', { params }), api.get('/records/summary', { params: sp })])
    records.value = rr.data?.records || []
    page.value = 1
    summaryDgmv.value = sr.data?.totalDgmv || 0
  } catch { /* empty */ }
  loading.value = false
}
...
async function fetchUnfilled() {
  try {
    const res = await api.get('/records/unfilled', { params: { date: yesterday } })
...
async function openWebhookModal() {
  ...
  webhookMsg.value = ''
  try {
    const res = await api.get('/webhook/record_unfilled')
...
async function saveWebhook() {
  ...
  webhookMsg.value = ''
  try {
    await api.put('/webhook/record_unfilled', { url: webhookUrl.value })
...
onMounted(() => {
  if (canViewAll.value) { fetchUnfilled(); fetchWebhookStatus() }
  fetchRecords()
})
</script>
